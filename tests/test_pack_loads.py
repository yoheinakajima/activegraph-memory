...
def test_pack_declares_memory_objects_and_relations():
    assert len(pack.object_types) == 25
    ...
    assert "memory_ingestion_stage" in OBJECT_TYPE_NAMES
    ...
    assert "memory_source_turn" in OBJECT_TYPE_NAMES
    assert "memory_conflict" in OBJECT_TYPE_NAMES
    assert "memory_retrieval_assessment" in OBJECT_TYPE_NAMES
...
def test_default_settings_match_phase_one_contract():
    ...
    assert settings.enable_claim_extraction is True
    assert settings.enable_temporal_resolution is True
